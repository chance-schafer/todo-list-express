...
const app = express() // setting the express call to the constant 'app'
const MongoClient = require('mongodb').MongoClient // makes it possible to use methods associated with MongoClient and talk to our DB
const PORT = 2121 // setting a constant to define the location our server will be listening.
...
let db, // declare a variable called db but not assigning a value
    ...
    dbName = 'todo' // declaring a variable and assiging the name of our database we will be using
...
MongoClient.connect(dbConnectionStr, { useUnifiedTopology: true }) // creating a connection to MongoDB, and passing in our connection string. Also passing in an additional property
    .then(client => { // waiting for connection and processing if successful, and passing in all client information
...
app.put('/markUnComplete', (request, response) => { // starts a PUT method when the markUncomplete route is passed in
    ...
    .then(result => { // starts a then if update was successful
        ...
        response.json('Marked Complete') // sending a resposne back to the sender
    }) // closing .then
    .catch(error => console.error(error)) // catching errors
...
app.delete('/deleteItem', (request, response) => {// starts a DELETE method when the delete route is passed in
    db.collection('todos').deleteOne({thing: request.body.itemFromJS}) // look in the db for one item matching the name from our JS file
